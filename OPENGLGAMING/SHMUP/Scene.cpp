...
Scene::Scene()
{
	...
	// 	{
	// 		vertices[(i * height + j) * 8] = (float)i / (float)width + 0.02;
	// 		vertices[(i * height + j) * 8 + 1] = (float)heightMap[(i * height + j) * 3] / 255.0f * 0.2;
	// 		vertices[(i * height + j) * 8 + 2] = (float)j / (float)height + 0.02;
	//
...
void Scene::Update(float dt)
{
	...
	// modelMatrix = translationMatrix * glm::rotate(modelMatrix, glm::radians(45.0f * dt), rotationAxisX) * scaleMatrix;
	// mvp = projectionMatrix * viewMatrix * modelMatrix;
